...
- 2026-05-26 후속 수정에서 `POST /api/notifications/generate-due`를 `X-Internal-Job-Token` 기반 내부 작업 토큰으로 보호하고, 응답을 생성 개수로 축소했다.
- 2026-05-26 후속 수정에서 알림 로그 생성을 PostgreSQL `ON CONFLICT DO NOTHING` 기반 insert로 변경해 중복 생성 충돌을 흡수하도록 했다.
