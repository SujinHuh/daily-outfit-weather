...
- 실제 운영 도메인 배포 시 Google Console redirect URI를 운영 도메인으로 등록해야 한다.
- 운영 플랫폼이 TLS를 종료한다면 proxy가 `X-Forwarded-Proto: https`를 전달해야 한다.
- KMA location grid catalog는 아직 전국 전체 데이터가 아니므로, 서비스 지역을 현재 catalog 지원 범위로 제한하거나 catalog 확장이 필요하다.
- 로그는 컨테이너 볼륨에 남도록 구성했지만, 운영 로그 수집/보관/알림 시스템은 별도 인프라에서 연결해야 한다.
- 운영 DB 백업/복구/마이그레이션 롤백 절차는 아직 별도 문서화가 필요하다.
